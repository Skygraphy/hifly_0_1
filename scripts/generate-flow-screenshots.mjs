...
try {
  ...
  await page.getByTestId("account-menu-settings-link").click();
  await page.waitForURL(/\/settings$/);
  await page.waitForTimeout(200);
  await shot(page, "Konto-Einstellungen", "Eigene Einstellungen, sichtbare Auswahl hängt von der Rolle ab");

  await page.getByTestId("account-menu-trigger").click();
  await page.getByTestId("account-menu-app-settings-link").click();
  await page.waitForURL(/\/admin\/settings$/);
  await page.waitForTimeout(200);
  await shot(page, "App-Einstellungen", "Nur super_admin: globale, sitezweite Einstellungen");
...
// Navigationskarte: von Hand gepflegt (zeigt mögliche Wege, keine echten
// Klicks). Als eigene Konstante statt inline im Report-Template, damit die
// Drift-Erkennung unten die enthaltenen Routen aus derselben Quelle lesen
// kann, die auch tatsächlich gerendert wird.
const DIAGRAM_SVG = `<svg viewBox="0 0 870 900" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Navigationsdiagramm der HiFly-App">
        <defs>
        ...
        <path class="edge" d="M470,150 H 830 V 793"/>
        <text class="edge-label" x="836" y="280" transform="rotate(90 836 280)" text-anchor="middle">Konto-Menü</text>
        <path class="edge" d="M830,683 H 470" marker-end="url(#arrow)"/>
        <text class="edge-label" x="490" y="675">Menü → Konto-Einstellungen (alle eingeloggten Rollen)</text>
        <rect class="node-box" x="250" y="650" width="220" height="66"/>
        <text class="node-path" x="270" y="678">/settings</text>
        <text class="node-label" x="270" y="700">Konto-Einstellungen</text>
        <path class="edge" d="M830,793 H 470" marker-end="url(#arrow)"/>
        <text class="edge-label" x="490" y="785">Menü → App-Einstellungen (super_admin)</text>
        <rect class="node-box" x="250" y="760" width="220" height="66"/>
        <text class="node-path" x="270" y="788">/admin/settings</text>
        <text class="node-label" x="270" y="810">App-Einstellungen</text>
      </svg>`;

// Drift-Erkennung: vergleicht die tatsächlichen Routen im Projekt
// (src/app/**/page.tsx) gegen das, was der Walkthrough besucht bzw. die
// Navigationskarte zeigt — damit fällt sofort auf, wenn der Report der App
// hinterherhinkt, statt dass er still weiter dasselbe Ergebnis liefert.
...
function extractDiagramRoutes(svg) {
  return [...svg.matchAll(/class="node-path"[^>]*>([^<]+)</g)].map((m) => m[1].trim());
}

const discoveredRoutes = discoverAppRoutes(path.join(process.cwd(), "src", "app"));
const visitedRoutes = [...new Set(steps.map((s) => new URL(s.url).pathname))];
const diagramRoutes = extractDiagramRoutes(DIAGRAM_SVG);
const missingFromWalkthrough = discoveredRoutes.filter((r) => !visitedRoutes.includes(r));
const missingFromDiagram = discoveredRoutes.filter((r) => !diagramRoutes.includes(r));

if (missingFromWalkthrough.length || missingFromDiagram.length) {
  console.log("\n⚠ Drift erkannt:");
  for (const r of missingFromWalkthrough) console.log(`  ${r} — fehlt im Walkthrough`);
  for (const r of missingFromDiagram) console.log(`  ${r} — fehlt in der Navigationskarte`);
}
...
function buildReportHtml(steps, generatedAt, drift) {
...
<style>
  :root {
    ...
    --warn: #9a5b00; --warn-soft: rgba(217, 152, 6, 0.08);
  }
  @media (prefers-color-scheme: dark) {
    :root {
      ...
      --warn: #fbbf24; --warn-soft: rgba(251, 191, 36, 0.1);
    }
  }
  ...
  * { box-sizing: border-box; }
  ...
  .wrap { max-width: 900px; margin: 0 auto; padding: 56px 24px 96px; display: flex; flex-direction: column; gap: 64px; }
  ...
  .drift-banner { background: var(--warn-soft); border: 1px solid var(--warn); border-radius: 12px; padding: 16px 20px; }
  .drift-title { font-weight: 700; color: var(--warn); margin: 0 0 6px; }
  .drift-desc { color: var(--text); margin: 0 0 10px; font-size: 0.92rem; }
  .drift-list { margin: 0; padding-left: 1.2em; font-size: 0.9rem; }
  .drift-list li { margin: 2px 0; }
  ...
  .diagram-scroll { overflow-x: auto; max-width: 760px; margin: 0 auto; border: 1px solid var(--border); border-radius: 12px; background: var(--surface); }
  ...
  /* .steps teilt sich dieselbe zentrierte 760px-Box wie .diagram-scroll,
     damit beide Bereiche exakt dieselben linken/rechten Kanten haben. Die
     nummerierte Schiene hängt sich per position:absolute links AUSSERHALB
     dieser Box in die Gutter — sie verbraucht keine Breite der Box selbst,
     sonst würde die Karte schmaler und nach rechts verschoben, statt mit
     dem Diagramm übereinzustimmen. */
  .steps { max-width: 760px; margin: 0 auto; display: flex; flex-direction: column; gap: 0; position: relative; }
  .step { position: relative; padding-bottom: 36px; }
  .step:last-child { padding-bottom: 0; }
  .step-rail { position: absolute; left: -64px; top: 0; bottom: 0; width: 44px; display: flex; flex-direction: column; align-items: center; }
...
<div class="wrap">
  <header>
    ...
    <div class="meta">
      <span class="tag">${drift.totalRoutes} Seiten</span>
...
function buildDriftBannerHtml(drift) {
  const items = [
    ...drift.missingFromWalkthrough.map((route) => ({ route, reason: "fehlt im Walkthrough" })),
    ...drift.missingFromDiagram.map((route) => ({ route, reason: "fehlt in der Navigationskarte" })),
  ];
  if (items.length === 0) return "";

  const list = items
    .map((i) => `<li><code>${escapeHtml(i.route)}</code> — ${escapeHtml(i.reason)}</li>`)
    .join("\n        ");

  return `<div class="drift-banner">
    <p class="drift-title">⚠ Dieser Report hinkt der App hinterher</p>
    <p class="drift-desc">Diese Routen existieren im Projekt (<code>src/app/**/page.tsx</code>), sind aber noch nicht in Walkthrough bzw. Navigationskarte eingepflegt:</p>
    <ul class="drift-list">
        ${list}
    </ul>
  </div>`;
